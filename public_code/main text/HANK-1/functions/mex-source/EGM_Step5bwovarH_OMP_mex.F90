...
!--------------------------------------------------------------------------------
	...
	subroutine egm_step5b_aux(nm,nk,ns,nh,ncapH,ncapM,&
		...
		real(8),intent(in)      :: P_vH(ncapH,ncapH,ns)
			...
			do i2=1,ncapM
				do i3=1,ns*nh
					do inext=1,ns*nh
						trprob=P(i3,inext)
						do i4=1,nk
!$OMP PARALLEL DO SHARED(tprob,input,i1,i2,i3,i4) PRIVATE(i5)  
							do i5=1,nm
								temp(i5,i4,i3,i2,i1)=&
...
!$OMP END PARALLEL DO 
		...
		do i1=1,ncapH
			do i2=1,ncapM
				do i3=1,ns
					do Hnext=1,ncapH
						hprob=P_vH(i1,Hnext,i3)
						do Mnext=1,ncapM
							mprob=P_M(i2,Mnext,i3,i1)
							do i4=1,nh
								shind=ns*(i4-1)+i3
								do i5=1,nk
!$OMP PARALLEL DO SHARED(temp,output,shind,i1,i2,i3,i4,i5,Hnext,Mnext,hprob,mprob) PRIVATE(i6) 
									do i6=1,nm
