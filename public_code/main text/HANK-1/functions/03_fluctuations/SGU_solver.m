function [hx,gx,F1,F2,F3,F4,par] = SGU_solver(F, numstates,numcontrols,Gamma_state,oc,mpar,par,grid)
...
% pool='MJSProfile1' % select machine

if isempty(p)
    parpool(pool,2) % total = 26, divisible by 2 (not 10) 
end
p = gcp('nocreate');
...
%on Value function error (LAST THREE COLUMNS TO BE FILLED: Aggregate Prices)
...
parfor bl=1:blocks
    ...
    if sum(range==numstates-1)==1
        ...
        h = par.scaleval2;
        X(Xct1) = h;
        Fx = F(ss,X,cc,cc);%Fsys(S_sp,X,C_sp,Cm_sp,Xss,Yss,Gamma_state,Gamma_control,InvGamma,par,mpar,grid,targets);
        DF3(:,Xct1-(bl-1)*packagesize)= (Fx-Fb)/h;
        Fx = F(X,ss,cc,cc);%Fsys(S_sp,Sm_sp,C_sp,Cm_sp,Xss,Yss,Gamma_state,Gamma_control,InvGamma,par,mpar,grid,targets);
        DF1(:,Xct1-(bl-1)*packagesize)= (Fx-Fb)/h;
    end
...
%  F2((mpar.nm-1):numstates-2,:)=0;

 
%% Remove numerical Garbage II: B and K should be moved according to grid.m and grid.k
% [B;K]=[grid.m 0  0;0 grid.k 0]*Gamma_state*X(Distr)
% F1(end, 1:numstates-2)=-[grid.m zeros(1,mpar.nh)]*Gamma_state;
% F3(end-1:end, 1:numstates-2)=0;%-[grid.m zeros(1,mpar.nk+mpar.nh)]*Gamma_state;
% F1(end-1, 1:numstates-2)=-[zeros(1,mpar.nm) zeros(1,mpar.nh)]*Gamma_state;
