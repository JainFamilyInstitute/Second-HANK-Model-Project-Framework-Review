...
!function [ psi_new ] =  EGM_Step5b(E_rhs_mu,m_n_star,psi_guess,grid,idm_n,P,beta,AProb,REP)
!----------------------------------------------------------------------------
! Gateway routine
!----------------------------------------------------------------------------
...
!--------------------------------------------------------------------------------
    ...
    real(8),intent(in)      :: AProb(nm,nk,nh)
		...
		real(8),intent(out)     :: psi_new(nm,nk,nh)
		 ...
		 ! Local variables
		integer(4)              :: i3,i4,i5,i1
		integer(4)              :: idx
		real(8)     :: dist_m_n(nm,nk,nh)
		real(8)     :: mu_flow(nm,nk,nh)
		real(8)     :: aux(nm,nk,nh)
		real(8)     :: ww(nm,nk,nh)
		real(8)			:: aux1, aux2
	...
	! subroutines: one for expectations/interpolation (first sh then PM)
     subroutine interpolate_m_n(nm,nk,nh,&
        ...
        real(8),intent(in)      :: dist_m_n(nm,nk,nh)
        integer(4),intent(in)   :: idm_n(nm,nk,nh)
        ! local variables
        integer(4)              :: i3,i4,i5
        integer(4)              :: idx
        ...
        ! interpolation over policies
                ...
                do i3=1,nh
                    do i4=1,nk
                        do i5=1,nm
                            idx=idm_n(i5,i4,i3)
                            aux1=input(idx,i4,i3)
                            aux2=input(idx+1,i4,i3)
                            output(i5,i4,i3)=aux1+&
                                dist_m_n(i5,i4,i3)*(aux2-aux1)
                        end do
                    end do
                end do
	...
	subroutine expectations(nm,nk,nh,&
		output,input,P,beta,AProb)
    ...
    real(8)     :: input_loc(nm,nk,nh)
		...
		CAProb=1.d0
    CAProb=CAProb-AProb
    input_loc=CAProb*input
    ! 1. Take expectations over sxh
		aux=reshape(input_loc,(/nm*nk,nh/))
