function [c_a_star,m_a_star,k_a_star,c_n_star,m_n_star,nnminus,naminus]=EGM_policyupdate_MIT(EVm,EVk,Qminus,PIminus,RBminus,Wminus,Profitminus,Tauminus,inc,mesh,grid,par,mpar)
mutil = @(c)(1./(c.^par.xi));
...
% while distC>1e-4
nnminus= ((1/par.gamma).*mutil(c_new(:)).*mesh.h(:).*Tauminus.*Wminus).^(par.sigma_n);
...
for jj=1:sum(binding_constraints(:))
    bbind=bbaux(jj);
    fcons= @(c_constr)cons_at_constraint(c_constr,mesh.h(bbind),Resource(bbind)-grid.m(1),par,mutil,Wminus);
    [c_constr, excess]=fzero(fcons,c_update(bbind),options);
    c_n_star(bbind)=c_constr;
end

% distC=max(abs(c_n_star(:)-c_new(:)));
% c_new=c_n_star;
% end
...
naminus= ((1/par.gamma).*mutil(c_a_aux(:)).*meshhaux(:).*Tauminus.*Wminus).^(par.sigma_n);
...
for j=1:mpar.nh
    ...
    if m_star_zero(j)>grid.m(1)
        ...
        n_a_guess= ((1/par.gamma)*mutil(c_k_cons).*grid.h(j).*Tauminus.*Wminus).^(par.sigma_n);
...
for jj=1:sum(log_index(:))
    bbind=bbaux(jj);
    fcons= @(c_constr)cons_at_constraint(c_constr,mesh.h(bbind),Resource_grid(bbind,j)-grid.m(1),par,mutil,Wminus);
