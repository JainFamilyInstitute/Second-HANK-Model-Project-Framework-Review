function [ N,R_fc,W_fc,Profits_fc,WW,RR,RBRB,Y ] = factor_returns_guess(meshes,grid,par,mpar)
%factor_returns
  
    mc =  par.mu - (par.beta * log(par.PI) - log(par.PI))/par.kappa;
        
%%  GHH Preferences    
    N    =  0.5;%(par.tau.*par.alpha.*grid.K.^(1-par.alpha).*mc).^(1/(1-par.alpha+par.gamma));   
    ...
    NW=(N/par.H).*W_fc;
    WW=NW*ones(mpar.nm,mpar.nk,mpar.ns*mpar.nh); %Wages
