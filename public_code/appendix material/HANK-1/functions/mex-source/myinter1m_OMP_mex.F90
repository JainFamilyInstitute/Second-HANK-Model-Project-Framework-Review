...
!$OMP PARALLEL DO SHARED(fxi1,fx1, xi,x) PRIVATE(i1,ind,weight) SCHEDULE(GUIDED) 
	do i1 = 1,nxi
...
subroutine myinter3_3inp(nx,nxi,x,xi,fx1,fx2,fx3,fxi1,fxi2,fxi3)
	...
	! Loop over xi and interpolate
!$OMP PARALLEL DO SHARED(fxi1,fxi2,fxi3, fx1,fx2,fx3, xi,x) PRIVATE(i1,ind,weight) SCHEDULE(GUIDED)
	do i1 = 1,nxi
		!ind = max(min(int((xi(i1,i2,i3)-x(1))/incr)+1,nx-1),1)
		call locate(x,nx,xi(i1),ind)
		ind = max(min(ind,nx-1),1)
...
subroutine myinter3_2inp(nx,nxi,x,xi,fx1,fx2,fxi1,fxi2)
	...
	! Loop over xi and interpolate
!$OMP PARALLEL DO SHARED(fxi1,fxi2, fx1,fx2, xi,x) PRIVATE(i1,ind,weight) SCHEDULE(GUIDED)
	do i1 = 1,nxi
				!ind = max(min(int((xi(i1,i2,i3)-x(1))/incr)+1,nx-1),1)
				call locate(x,nx,xi(i1),ind)
				ind = max(min(ind,nx-1),1)
